...
namespace QLStudy.Service.Api.Features.Legacy
{
    ...
    public class SemestersControllerLogic : BaseApiController
    {
        public SemestersControllerLogic(QLStudyDbContext context) : base(context)
        {
        }
        ...
        public async Task<IActionResult> GetTuitionMatrix(int id)
        {
            ...
            var enrollmentLookup = enrollments
                ...
                .ToDictionary(g => g.Key, g => g.OrderBy(e => ResolveMonthForClassRange(e.StartMonth, g.First().Class?.StartDate, null)).ThenBy(e => e.CreatedAt).ToList());
            ...
            var studentRows = studentClasses.Select(sc =>
            {
                ...
                var rowEnrollments = enrollmentLookup.TryGetValue($"{sc.StudentId}:{sc.ClassId}", out var foundEnrollments)
                    ? foundEnrollments
                    : new List<StudentClassEnrollment>();
                var payablePeriodIds = rowEnrollments.Any()
                    ? GetPayablePeriodIdsForEnrollments(periods, sc.Class!.StartDate, sc.Class!.EndDate, rowEnrollments)
                    : GetPayablePeriodIds(periods, sc.Class!.StartDate, sc.Class!.EndDate, startMonth);
                ...
                return new
                {
                    ...
                    classPeriodIds,
                    payablePeriodIds,
                    enrollmentStatus = rowEnrollments.OrderByDescending(e => e.Status == "Active").ThenByDescending(e => e.CreatedAt).FirstOrDefault()?.Status ?? "Active",
                    enrollments = rowEnrollments.Select(e => new { e.StartMonth, e.EndMonth, e.Status, e.Reason }).ToList(),
        ...
        private async Task EnsureTuitionPeriodsForClassDateRanges(int semesterId)
        {
            ...
            classes = classes
                .Where(c => IsReasonableClassDate(c.StartDate!.Value)
                    && IsReasonableClassDate(c.EndDate!.Value)
                    && c.EndDate.Value >= c.StartDate.Value
                    && MonthDistance(c.StartDate.Value, c.EndDate.Value) <= 36)
                .ToList();
        ...
        private static List<int> GetPayablePeriodIdsForEnrollments(IEnumerable<dynamic> periods, DateOnly? classStartDate, DateOnly? classEndDate, List<StudentClassEnrollment> enrollments)
        {
            var result = new SortedSet<int>();
            var maxPeriodStart = periods
                ...
                .DefaultIfEmpty(new DateOnly(DateTime.UtcNow.Year, DateTime.UtcNow.Month, 1))
                .Max();
            ...
            foreach (var enrollment in enrollments)
            {
                ...
                var end = string.IsNullOrWhiteSpace(enrollment.EndMonth)
                    ? (classEndDate == null ? (DateOnly?)null : new DateOnly(classEndDate.Value.Year, classEndDate.Value.Month, 1))
                    : ResolveMonthForClassRange(enrollment.EndMonth!, classStartDate, classEndDate);

                if (start == null) continue;
                if (end == null)
                {
                    end = classEndDate == null ? maxPeriodStart : new DateOnly(classEndDate.Value.Year, classEndDate.Value.Month, 1);
                }

                if (end.Value > maxPeriodStart)
                {
                    end = maxPeriodStart;
                }

                foreach (var periodId in GetPeriodIdsForDateRange(periods, start.Value, end.Value))
                {
                    result.Add(periodId);
                }
            }
